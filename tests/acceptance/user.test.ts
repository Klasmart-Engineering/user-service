...
describe('acceptance.user', () => {
    ...
    context('userNode', () => {
        ...
        beforeEach(async () => {
            myUser = await createUser().save()
            myOrg = await createOrganization().save()
            mySchool = await School.save(createSchool(myOrg))
        ...
        it('gets the queried user node', async () => {
            const response = await request
                .post('/graphql')
                .set({
                    ContentType: 'application/json',
            ...
            const userNode = response.body.data.userNode

            expect(response.status).to.eq(200)
            expect(userNode.id).to.equal(myUser.user_id)
        })

        it('gets the organizations of the queried user', async () => {
            const response = await request
                .post('/graphql')
                .set({
                    ContentType: 'application/json',
            ...
            expect(userNode.organizations.length).to.equal(1)
            expect(userNode.organizations[0].name).to.equal(
                myOrg.organization_name
            )
        })

        it('gets the schools of the queried user', async () => {
            const response = await request
                .post('/graphql')
                .set({
                    ContentType: 'application/json',
            ...
            expect(userNode.schools.length).to.equal(1)
            expect(userNode.schools[0].name).to.equal(mySchool.school_name)
        })

        it('responds with an error if the entity does not exist', async () => {
            const response = await request
                .post('/graphql')
                .set({
                    ContentType: 'application/json',
