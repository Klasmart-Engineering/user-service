...
export default function (roleType?: string) {
    let res = http.get(`${process.env.ASSESSMENT_STUDENT_URL}?complete_at_ge=1643287874&complete_at_le=1644497474&order_by=-complete_at&org_id=${process.env.ORG_ID}&page=1&page_size=5&type=home_fun_study` as string, params);

    if (res.status === 401) {
        http.get(`https://auth.${process.env.APP_URL}/refresh`, params);
        res = http.get(`${process.env.ASSESSMENT_STUDENT_URL}?complete_at_ge=1643287874&complete_at_le=1644497474&order_by=-complete_at&org_id=${process.env.ORG_ID}&page=1&page_size=5&type=home_fun_study` as string, params);
    }
