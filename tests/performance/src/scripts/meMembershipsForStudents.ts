...
export default function (roleType?: string) {
    ...
    // Payload for the query meMemership "1" - keep this payload
    const membershipPayload1 = JSON.stringify({
        variables: {},
        query: meMembershipForStudent1, 
    });

    // Payload for the query meMemership "2" - keep this payload
    const membershipPayload2 = JSON.stringify({
        variables: {},
        query: meMembershipForStudent2, 
    });

     // Payload for the query meMemership "3" - keep this payload
     const membershipPayload3 = JSON.stringify({
        variables: {},
        query: meMembershipForStudent3,
    });

     // Payload for the query meMemership "4"  - keep this payload
     const membershipPayload4 = JSON.stringify({
        variables: {},
        query: meMembershipForStudent4,
    });

    // The query below was removed in the last deployment - Landing schedule section - April 19, 2020
   /*    // Payload for the query meMemership "5" 
    ...
    // Payload for the query meMemership "5"  - keep this payload
    ...
    // The lines below that are commented is related to the meQueryBasic
 ...
 */
    ...
    check(res, {
        ...
        '"Get me membership 1" query returns data': (r) => JSON.parse(r.body as string).data?.meMembership?.membership !== undefined, // expected: true
    ...
    check(res2, {
        ...
        '"Get me membership 2" query returns data': (r) => JSON.parse(r.body as string).data?.meMembership?.membership !== undefined, // expected: true
    ...
    check(res3, {
       ...
       '"Get me membership 3" query returns data': (r) => JSON.parse(r.body as string).data?.meMembership?.membership !== undefined, // expected: true
   ...
   */   
    ...
    check(res4, {
        ...
        '"Get me membership 4" query returns data': (r) => JSON.parse(r.body as string).data?.meMembership?.membership !== undefined, // expected: false
    ...
    check(res5, {
        ...
        '"Get me membership 5" query returns data': (r) => JSON.parse(r.body as string).data?.meMembership?.membership !== undefined, // expected: false
