...
// For Class K6 035
const studyClassPayload32 = {
    ...studyClassPayload, "class_id": process.env.CLASS_ID_STUDY_SETUP32,
}

// For Class K6 036
const studyClassPayload33 = {
    ...studyClassPayload, "class_id": process.env.CLASS_ID_STUDY_SETUP33,
}

// For Class K6 037
const studyClassPayload34 = {
    ...studyClassPayload, "class_id": process.env.CLASS_ID_STUDY_SETUP34,
}

// For Class K6 038
const studyClassPayload35 = {
    ...studyClassPayload, "class_id": process.env.CLASS_ID_STUDY_SETUP35,
}

// For Class K6 039
const studyClassPayload36 = {
    ...studyClassPayload, "class_id": process.env.CLASS_ID_STUDY_SETUP36,
}
...
export function setup() {
    ...
    const classRosterData32 = getClassRosterTest(process.env.ORG_ID as string, process.env.CLASS_ID_STUDY_SETUP32 as string);
    const classRosterData33 = getClassRosterTest(process.env.ORG_ID as string, process.env.CLASS_ID_STUDY_SETUP33 as string);
    const classRosterData34 = getClassRosterTest(process.env.ORG_ID as string, process.env.CLASS_ID_STUDY_SETUP34 as string);
    const classRosterData35 = getClassRosterTest(process.env.ORG_ID as string, process.env.CLASS_ID_STUDY_SETUP35 as string);
    const classRosterData36 = getClassRosterTest(process.env.ORG_ID as string, process.env.CLASS_ID_STUDY_SETUP36 as string);
    ...
    const classPayload32 = {
        ...studyClassPayload32,
        class_roster_student_ids: classRosterData32.class.students.map((student: any) => student.user_id),
        class_roster_teacher_ids: classRosterData32.class.teachers.map((teacher: any) => teacher.user_id),
    };

    const classPayload33 = {
        ...studyClassPayload33,
        class_roster_student_ids: classRosterData33.class.students.map((student: any) => student.user_id),
        class_roster_teacher_ids: classRosterData33.class.teachers.map((teacher: any) => teacher.user_id),
    };

    const classPayload34 = {
        ...studyClassPayload34,
        class_roster_student_ids: classRosterData34.class.students.map((student: any) => student.user_id),
        class_roster_teacher_ids: classRosterData34.class.teachers.map((teacher: any) => teacher.user_id),
    };

    const classPayload35 = {
        ...studyClassPayload35,
        class_roster_student_ids: classRosterData35.class.students.map((student: any) => student.user_id),
        class_roster_teacher_ids: classRosterData35.class.teachers.map((teacher: any) => teacher.user_id),
    };

    const classPayload36 = {
        ...studyClassPayload36,
        class_roster_student_ids: classRosterData36.class.students.map((student: any) => student.user_id),
        class_roster_teacher_ids: classRosterData36.class.teachers.map((teacher: any) => teacher.user_id),
    };
    ...
    const res32 = http.post(`${process.env.SCHEDULES_URL}?org_id=${process.env.ORG_ID}`, JSON.stringify(classPayload32), params);
    const res33 = http.post(`${process.env.SCHEDULES_URL}?org_id=${process.env.ORG_ID}`, JSON.stringify(classPayload33), params);
    const res34 = http.post(`${process.env.SCHEDULES_URL}?org_id=${process.env.ORG_ID}`, JSON.stringify(classPayload34), params);
    const res35 = http.post(`${process.env.SCHEDULES_URL}?org_id=${process.env.ORG_ID}`, JSON.stringify(classPayload35), params);
    sleep(1.5);
    const res36 = http.post(`${process.env.SCHEDULES_URL}?org_id=${process.env.ORG_ID}`, JSON.stringify(classPayload36), params);
    ...
    // res 32
    if (!JSON.parse(res32.body as string).data?.id) {
        throw new Error('Class ID not setup, aborting test!' + JSON.stringify(res32));
    } else {
        console.log('Class created successfully with ID: ', JSON.parse(res32.body as string).data?.id);
    }

     // res 33
     if (!JSON.parse(res33.body as string).data?.id) {
        throw new Error('Class ID not setup, aborting test!' + JSON.stringify(res33));
    } else {
        console.log('Class created successfully with ID: ', JSON.parse(res33.body as string).data?.id);
    }

      // res 34
      if (!JSON.parse(res34.body as string).data?.id) {
        throw new Error('Class ID not setup, aborting test!' + JSON.stringify(res34));
    } else {
        console.log('Class created successfully with ID: ', JSON.parse(res34.body as string).data?.id);
    }

    // res 35
    if (!JSON.parse(res35.body as string).data?.id) {
        throw new Error('Class ID not setup, aborting test!' + JSON.stringify(res35));
    } else {
        console.log('Class created successfully with ID: ', JSON.parse(res35.body as string).data?.id);
    }

    // res 36
    if (!JSON.parse(res36.body as string).data?.id) {
        throw new Error('Class ID not setup, aborting test!' + JSON.stringify(res36));
    } else {
        console.log('Class created successfully with ID: ', JSON.parse(res36.body as string).data?.id);
    }
    ...
    data = {
        ...
        classId32: JSON.parse(res32.body as string).data?.id,
        classId33: JSON.parse(res33.body as string).data?.id,
        classId34: JSON.parse(res34.body as string).data?.id,
        classId35: JSON.parse(res35.body as string).data?.id,
        classId36: JSON.parse(res36.body as string).data?.id,
    }
...
// seguir agregando clases
//Class K6 035 students30
export function students30(data: { [key: string]: { res: any, userId: string } }) {
    //const random = randomNumber(22);
    const random = `7${('0' + randomNumber(22)).slice(-2)}`;
    const jar = http.cookieJar();
    jar.set(process.env.COOKIE_URL as string, 'access', data[`student${random}`].res.cookies?.access[0].Value, {
        domain: process.env.COOKIE_DOMAIN,
    });
    ...
    const token = getLiveClassToken(data.classId32 as unknown as string);

    const liveClassPayload32 = {
        token,
        refreshId: refreshId as unknown as string,
        roomId: data.classId32 as unknown as string,
        accessCookie: data[`student${random}`].res.cookies?.access[0].Value,
        userId: data[`student${random}`].userId,
    };

    console.log(JSON.stringify(liveClassPayload32));

    studyClassWebSockets(liveClassPayload32);
}

//Class K6 036 students31
export function students31(data: { [key: string]: { res: any, userId: string } }) {
    //const random = randomNumber(22);
    const random = `7${('0' + randomNumber(22)).slice(-2)}`;
    const jar = http.cookieJar();
    jar.set(process.env.COOKIE_URL as string, 'access', data[`student${random}`].res.cookies?.access[0].Value, {
    ...
    const token = getLiveClassToken(data.classId33 as unknown as string);

    const liveClassPayload33 = {
        token,
        refreshId: refreshId as unknown as string,
        roomId: data.classId33 as unknown as string,
        accessCookie: data[`student${random}`].res.cookies?.access[0].Value,
        userId: data[`student${random}`].userId,
    };

    console.log(JSON.stringify(liveClassPayload33));

    studyClassWebSockets(liveClassPayload33);
}

//Class K6 037 students32
export function students32(data: { [key: string]: { res: any, userId: string } }) {
    //const random = randomNumber(22);
    const random = `7${('0' + randomNumber(22)).slice(-2)}`;
    const jar = http.cookieJar();
    jar.set(process.env.COOKIE_URL as string, 'access', data[`student${random}`].res.cookies?.access[0].Value, {
    ...
    const token = getLiveClassToken(data.classId34 as unknown as string);

    const liveClassPayload34 = {
        token,
        refreshId: refreshId as unknown as string,
        roomId: data.classId34 as unknown as string,
        accessCookie: data[`student${random}`].res.cookies?.access[0].Value,
        userId: data[`student${random}`].userId,
    };

    console.log(JSON.stringify(liveClassPayload34));

    studyClassWebSockets(liveClassPayload34);
}

//Class K6 038 students33
export function students33(data: { [key: string]: { res: any, userId: string } }) {
    //const random = randomNumber(22);
    const random = `7${('0' + randomNumber(22)).slice(-2)}`;
    const jar = http.cookieJar();
    jar.set(process.env.COOKIE_URL as string, 'access', data[`student${random}`].res.cookies?.access[0].Value, {
    ...
    const token = getLiveClassToken(data.classId35 as unknown as string);

    const liveClassPayload35 = {
        token,
        refreshId: refreshId as unknown as string,
        roomId: data.classId35 as unknown as string,
        accessCookie: data[`student${random}`].res.cookies?.access[0].Value,
        userId: data[`student${random}`].userId,
    };

    console.log(JSON.stringify(liveClassPayload35));

    studyClassWebSockets(liveClassPayload35);
}

//Class K6 039 students34
export function students34(data: { [key: string]: { res: any, userId: string } }) {
    //const random = randomNumber(22);
    const random = `7${('0' + randomNumber(22)).slice(-2)}`;
    const jar = http.cookieJar();
    jar.set(process.env.COOKIE_URL as string, 'access', data[`student${random}`].res.cookies?.access[0].Value, {
    ...
    const token = getLiveClassToken(data.classId36 as unknown as string);

    const liveClassPayload36 = {
        token,
        refreshId: refreshId as unknown as string,
        roomId: data.classId36 as unknown as string,
        accessCookie: data[`student${random}`].res.cookies?.access[0].Value,
        userId: data[`student${random}`].userId,
    };

    console.log(JSON.stringify(liveClassPayload36));

    studyClassWebSockets(liveClassPayload36);
}