...
import { addRoleToSchoolMembership, addRolesToSchoolMembership, removeRoleToSchoolMembership, leaveSchool } from "../utils/operations/schoolMembershipOps";
...
describe("SchoolMembership", () => {
    ...
    beforeEach(async () => {
        ...
        schoolMembership = await getSchoolMembershipViaSchool(testClient, schoolId, userId, { authorization: getNonAdminAuthToken() });
    });
    ...
    describe("addRole", () => {
        ...
        context("when the school is active", () => {
            it("adds the role to the school membership", async () => {
                const role = await addRoleToSchoolMembership(testClient, userId, schoolId, roleId)
        ...
        context("when the school is inactive", () => {
            beforeEach(async () => {
                await leaveSchool(testClient, userId, schoolId, { authorization: getNonAdminAuthToken() });
            });
    ...
    describe("addRoles", () => {
        ...
        context("when the school is active", () => {
            it("adds the roles to the school membership", async () => {
                const roles = await addRolesToSchoolMembership(testClient, userId, schoolId, [roleId])
                const dbMembership = await SchoolMembership.findOneOrFail({ where: { user_id: userId, school_id: schoolId } });
                const dbRoles = await dbMembership.roles || [];
    ...
    describe("removeRole", () => {
        ...
        context("when the school is active", () => {
            it("removes the role to the school membership", async () => {
                const gqlMembership = await removeRoleToSchoolMembership(testClient, userId, schoolId, roleId)
                const dbMembership = await SchoolMembership.findOneOrFail({ where: { user_id: userId, school_id: schoolId } });
                const dbRoles = await dbMembership.roles || [];
    ...
    describe("leave", () => {
        ...
        context("when the school membership is active", () => {
            it("leaves the school membership", async () => {
                const leftGql = await leaveSchool(testClient, userId, schoolId, { authorization: getNonAdminAuthToken() });
                const dbMembership = await SchoolMembership.findOneOrFail({ where: { user_id: userId, school_id: schoolId } });
