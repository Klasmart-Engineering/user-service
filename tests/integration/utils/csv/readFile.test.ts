...
describe("read file", () => {
    ...
    context("when file data is empty", () => {
        ...
        const encoding = '7bit';     
        const dummyFn = (manager:EntityManager, row:any, rowCount: number ) => { return rowCount }
        it("should throw an error", async () => {
            const upload = {
                filename:filename,
                mimetype:mimetype,
                encoding:encoding, 
                createReadStream : () => { return fs.createReadStream(resolve(`tests/fixtures/empty.csv`)) }
            } as Upload
            const fn = async () => await readCSVFile(connection.manager,upload,[dummyFn])
