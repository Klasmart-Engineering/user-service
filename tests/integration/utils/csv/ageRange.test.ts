...
describe("processAgeRangeFromCSVRow", () => {
    ...
    context("when the age range low value is greater than age range high value", () => {
        beforeEach(() => {
            row = { ...row, age_range_low_value: String(Number(rowModel.age_range_high_value) + 1) }
        })
    ...
    context("when all data provided is valid", () => {
        it("creates an age range with its relations", async () => {
            ...
            expect(ageRange).to.exist;
            expect(ageRange.name).eq(`${rowModel.age_range_low_value} - ${rowModel.age_range_high_value} ${rowModel.age_range_unit}(s)`);
            ...
            expect(ageRange.high_value).eq(Number(rowModel.age_range_high_value));
            expect(ageRange.low_value_unit).eq(rowModel.age_range_unit);
            expect(ageRange.high_value_unit).eq(rowModel.age_range_unit);
            expect(organizationInAgeRange?.organization_id).eq(organization.organization_id);
        });
    });
});