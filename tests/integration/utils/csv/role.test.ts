...
describe("processRoleFromCSVRow", () => {
    ...
    context("when the provided permission already exists in the current role", () => {
        beforeEach(async () => {
            let permissions: Permission[] = [];
            const permissionFound = await Permission.findOneOrFail(row.permission_id);
            const role = new Role();
    ...
    context("when all data provided is valid", () => {
        it("creates the roles with its relations", async () => {
            ...
            expect(organizationInRole?.organization_name).eq(row.organization_name);
        });
    });
});
