...
import { subcategoriesConnection } from '../../utils/operations/modelOps'
...
describe('subcategoriesConnection', () => {
    ...
    context('pagination', () => {
        it('returns subcategories from all the list', async () => {
            ...
            expect(result.totalCount).to.eq(subcategoriesCount * 2)
        ...
        it("returns the subcategories that belongs to user's organization", async () => {
            ...
            expect(result.totalCount).to.eq(subcategoriesCount)
        ...
        context('linked data', () => {
            beforeEach(async () => {
                ...
                for (let i = 0; i < 60; i++) {
                    ...
                    const subject = createSubject(org2, [category])
                ...
                for (const subcategory of org2Subcategories) {
                    subcategory.categories = Promise.resolve(categories)
                }

                await connection.manager.save(org2Subcategories)
            })

            it("subcategories' linked data has not more than 50 elements per entity", async () => {
                const token = generateToken(userToPayload(orgOwner))
                const result = await subcategoriesConnection(
                    testClient,
                    'FORWARD',
        ...
        context('duplicated linked data', () => {
            ...
            it("subcategories' duplicated linked data is not shown", async () => {
                const token = generateToken(userToPayload(orgOwner))
                const result = await subcategoriesConnection(
                    testClient,
                    'FORWARD',
                ...
                const subcategories = result.edges
                subcategories.every((subcategory) => {
                    expect(subcategory.node.categories?.length).to.eq(1)
                    expect(subcategory.node.subjects?.length).to.eq(1)
                    expect(subcategory.node.programs?.length).to.eq(1)
                })
            })
        })
    })
    ...
    context('sorting', () => {
        it('returns subcategories sorted by id in and ascending order', async () => {
            const result = await subcategoriesConnection(
                testClient,
                'FORWARD',
                { count: 10 },
        ...
        it('returns subcategories sorted by id in and descending order', async () => {
            const result = await subcategoriesConnection(
                testClient,
                'FORWARD',
                { count: 10 },
        ...
        it('returns subcategories sorted by name in and ascending order', async () => {
            const result = await subcategoriesConnection(
                testClient,
                'FORWARD',
                { count: 10 },
        ...
        it('returns subcategories sorted by name in and descending order', async () => {
            const result = await subcategoriesConnection(
                testClient,
                'FORWARD',
                { count: 10 },
    ...
    context('filtering', () => {
        beforeEach(async () => {
            for (let i = 0; i < subcategoriesCount; i++) {
                org1Subcategories[i].status =
                    i % 2 ? Status.ACTIVE : Status.INACTIVE
            }

            await connection.manager.save(org1Subcategories)

            for (let i = 0; i < subcategoriesCount; i++) {
                org2Subcategories[i].system = !!(i % 2)
            }
