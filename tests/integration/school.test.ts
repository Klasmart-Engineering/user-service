...
describe("school", () => {
    ...
    describe("organization", () => {
        ...
        beforeEach(async () => {
            ...
            school = await createSchool(testClient, organizationId, "school 1", undefined, { authorization: getAdminAuthToken() })
        ...
        context("no permissions required", () => {
            it("should return the organization", async () => {
                const gqlOrganization = await getSchoolOrganization(testClient, schoolId, { authorization: getNonAdminAuthToken() });
    ...
    describe("classes", () => {
        ...
        context("no permissions required", () => {
            it("should return all classes", async () => {
                const gqlClasses = await getSchoolClasses(testClient, schoolId, { authorization: getNonAdminAuthToken() });
    ...
    describe("memberships", () => {
        ...
        context("no permissions required", () => {
            it("should return all memberships", async () => {
                const gqlMemberships = await getSchoolMembershipsViaSchool(testClient, schoolId, { authorization: getNonAdminAuthToken() });
    ...
    describe("membership", () => {
        ...
        context("no permissions required", () => {
            it("should return the membership", async () => {
                const gqlMembership = await getSchoolMembershipViaSchool(testClient, schoolId, userId, { authorization: getNonAdminAuthToken() });
    ...
    describe("set", () => {
        ...
        beforeEach(async () => {
            ...
            userId = (await createNonAdminUser(testClient)).user_id;
            organizationId = (await createOrganizationAndValidate(testClient, orgOwner.user_id, "org 1")).organization_id;
            school = await createSchool(testClient, organizationId, originalSchoolName,  undefined,{ authorization: getAdminAuthToken() })
            schoolId = school?.school_id;
            await addUserToOrganizationAndValidate(testClient, userId, organizationId, { authorization: getAdminAuthToken() });
            await addUserToSchool(testClient, userId, schoolId, { authorization: getAdminAuthToken() })
            roleId = (await createRole(testClient, organizationId, "test_role")).role_id;
            await grantPermission(testClient, roleId, PermissionName.edit_school_20330, { authorization: getAdminAuthToken() });
        });
            ...
            context("within the organization", () => {
                ...
                context("and the shortcode for the school has not been specified", () => {
                    it("does not modify the original shortcode", async () => {
                        const gqlSchool = await updateSchool(testClient, schoolId, originalSchoolName,  undefined,{ authorization: getNonAdminAuthToken() });
                ...
                it("should return the modified school and update the database entry", async () => {
                    const gqlSchool = await updateSchool(testClient, schoolId, newSchoolName,  "MYSHORT2",{ authorization: getNonAdminAuthToken() });
                 ...
                 it("should return the modified school and update the database entry ommiting the incorrect shortcode", async () => {
                    const gqlSchool = await updateSchool(testClient, schoolId, newSchoolName,  "myverwrong3",{ authorization: getNonAdminAuthToken() });
                ...
                context("and the school is marked as inactive", () => {
                    beforeEach(async () => {
                        await deleteSchool(testClient, school.school_id, { authorization: getAdminAuthToken() });
                    });

                    it("fails edit the school", async () => {
                        const gqlSchool = await updateSchool(testClient, schoolId, newSchoolName, undefined,{ authorization: getNonAdminAuthToken() });
        ...
        context("when user does not have the edit school permission", () => {
            it("should throw a permission exception, and not update the database entry", async () => {
                const fn = () => updateSchool(testClient, schoolId, newSchoolName, undefined, { authorization: getNonAdminAuthToken() });
    ...
    describe("addUser", () => {
        ...
        beforeEach(async () => {
            const orgOwner = await createAdminUser(testClient);
            idOfUserToPerformAction = (await createNonAdminUser(testClient)).user_id;
            idOfUserToBeAdded = (await createUserAndValidate(testClient, userToBeAdded)).user_id;
            organizationId = (await createOrganizationAndValidate(testClient, orgOwner.user_id, "org 1")).organization_id;
            school = await createSchool(testClient, organizationId, "school 1",  undefined,{ authorization: getAdminAuthToken() })
            schoolId = school?.school_id;
            await addUserToOrganizationAndValidate(testClient, idOfUserToPerformAction, organizationId, { authorization: getAdminAuthToken() });
            await addUserToSchool(testClient, idOfUserToPerformAction, schoolId, { authorization: getAdminAuthToken() })
            roleId = (await createRole(testClient, organizationId, "test_role")).role_id;
            await grantPermission(testClient, roleId, PermissionName.edit_school_20330, { authorization: getAdminAuthToken() });
        });
        ...
        context("when user has the edit school permission", () => {
            context("within the organization", () => {
                ...
                it("should return the membership and create a database entry", async () => {
                    await addUserToOrganizationAndValidate(testClient, idOfUserToBeAdded, organizationId, { authorization: getAdminAuthToken() });
                    const gqlMembership = await addUserToSchool(testClient, idOfUserToBeAdded, schoolId, { authorization: getNonAdminAuthToken() });
        ...
        context("when user does not have the edit school permission", () => {
            it("should throw a permission exception, and not add a database entry", async () => {
                ...
                const fn = () => addUserToSchool(testClient, idOfUserToBeAdded, schoolId, { authorization: getNonAdminAuthToken() });
                expect(fn()).to.be.rejected;
    ...
    describe("delete", () => {
        ...
        context("when authenticated", () => {
            context("and the user does not have delete class permissions", () => {
                ...
                it("should throw a permission exception, and not delete the database entry", async () => {
                    const fn = () => deleteSchool(testClient, school.school_id, { authorization: getNonAdminAuthToken() });
                    expect(fn()).to.be.rejected;
            ...
            context("and the user has all the permissions", () => {
                beforeEach(async () => {
                    const role = await createRole(testClient, organization.organization_id);
                    await grantPermission(testClient, role.role_id, PermissionName.delete_school_20440, { authorization: getAdminAuthToken() });
                    await addRoleToOrganizationMembership(testClient, user.user_id, organization.organization_id, role.role_id);
                });

                it("deletes the school", async () => {
                    const gqlSchool = await deleteSchool(testClient, school.school_id, { authorization: getNonAdminAuthToken() });
    ...
    describe("programs", () => {
        ...
        beforeEach(async () => {
            ...
            user = await createNonAdminUser(testClient);
            organization = await createOrganizationAndValidate(testClient, orgOwner.user_id);
            school = await createSchool(testClient, organization.organization_id, "school 1",  undefined,{ authorization: getAdminAuthToken() });
            const schoolId = school?.school_id
            program = createProgram(organization)
            await program.save()
            await editPrograms(testClient, school.school_id, [program.id], { authorization: getAdminAuthToken() });
            const organizationId = organization?.organization_id
            await addUserToOrganizationAndValidate(testClient, user.user_id, organization.organization_id, { authorization: getAdminAuthToken() });
        });
        ...
        context("when authenticated", () => {
            context("and the user does not have view class permissions", () => {
                it("fails to list programs in the school", async () => {
                    const fn = () => listPrograms(testClient, school.school_id, { authorization: getNonAdminAuthToken() });
            ...
            context("and the user has all the permissions", () => {
                beforeEach(async () => {
                    const role = await createRole(testClient, organization.organization_id);
                    await grantPermission(testClient, role.role_id, PermissionName.view_school_20110, { authorization: getAdminAuthToken() });
                    await addRoleToOrganizationMembership(testClient, user.user_id, organization.organization_id, role.role_id);
                });

                it("lists all the programs in the school", async () => {
                    const gqlPrograms = await listPrograms(testClient, school.school_id, { authorization: getNonAdminAuthToken() });
    ...
    describe("editPrograms", () => {
        ...
        context("when authenticated", () => {
            ...
            context("and the user does not have edit school permissions", () => {
                it("throws a permission error", async () => {
                    const fn = () => editPrograms(testClient, school.school_id, [program.id], { authorization: getNonAdminAuthToken() });
                    expect(fn()).to.be.rejected;
            ...
            context("and the user has all the permissions", () => {
                ...
                it("edits the school programs", async () => {
                    ...
                    let gqlPrograms = await editPrograms(testClient, school.school_id, [program.id], { authorization: getNonAdminAuthToken() });
                    ...
                    gqlPrograms = await editPrograms(testClient, school.school_id, [], { authorization: getNonAdminAuthToken() });
                    dbSchool = await School.findOneOrFail(school.school_id)
                    dbPrograms = await dbSchool.programs || []
                    expect(dbPrograms).to.be.empty
                });
                ...
                context("and the school is marked as inactive", () => {
                    ...
                    it("does not edit the school programs", async () => {
                        const gqlPrograms = await  editPrograms(testClient, school.school_id, [program.id], { authorization: getNonAdminAuthToken() });
