...
import {
    createUser,
    createUsers,
    createAdminUser as createAdminFactory,
...
import {
    ...
    AddRemoveOrganizationRolesEntityMap,
    AddRemoveOrganizationRolesInputType,
...
describe('user', () => {
    ...
    describe('add/remove organization roles', () => {
        ...
        function getMembershipRoles(
            input: AddRemoveOrganizationRolesInputType[]
        ) {
            return Promise.all(
                Array.from(input, (i) =>
        ...
        function getRemoveOrganizationRolesFromUsers(
            input: RemoveOrganizationRolesFromUserInput[],
            clientUser = admin
        ) {
            const permissions = new UserPermissions(userToPayload(clientUser))
            return new RemoveOrganizationRolesFromUsers(input, permissions)
        }
        ...
        async function checkNoChangesMade(
            ...
            const membershipRoles = await getMembershipRoles(input)
            input.forEach((_, index) => {
                const roles = membershipRoles[index]
                expect(roles).to.exist
                expect(roles).to.have.lengthOf(expectedLength)
            })
        }

        async function generateDataForEntityMaps() {
            const otherOrg = await createOrgFactory().save()
            const otherRoles = await Role.save(
                createRoles(rolesCount, otherOrg)
            )

            const otherUsers = await User.save(createUsers(usersCount))
        ...
        async function prepareDataForCheckMembershipRoles(
            orgMemberships: OrganizationMembership[],
            otherOrgMemberships: OrganizationMembership[],
            otherOrg: Organization,
            otherRoles: Role[],
            otherUsers: User[]
        ) {
            const expectedMap = new ObjMap<OrganizationMembershipKey, Role[]>()
            const allMemberships = [...orgMemberships, ...otherOrgMemberships]
            ...
            return { input, expectedMap }
        }

        function checkMembershipRoles(
            entityMaps: AddRemoveOrganizationRolesEntityMap,
            expectedMap: ObjMap<OrganizationMembershipKey, Role[]>
        ) {
            ...
            Array.from(membershipRoles.keys()).forEach((k) => {
                ...
                expect(mapValue?.map((v) => v.role_id)).to.deep.equalInAnyOrder(
                    expectedValue?.map((v) => v.role_id)
                )
            })
        }

        function commonValidateChecks(
            validate: (
                arg: AddRemoveOrganizationRolesInputType[]
            ) => Promise<APIError[]>
        ) {
        ...
        beforeEach(async () => {
            admin = await createAdminFactory().save()
            ...
            context('.authorize', () => {
                ...
                context('when caller does not have permissions', () => {
                    ...
                    it('returns a permission error', async () => {
                        ...
                        await checkNoChangesMade(input, 0)
                    })
                })
            })
        ...
        context('AddOrganizationRolesToUsers', () => {
            ...
            context('.run', () => {
                it('can add system roles to organization memberships', async () => {
                    const input = buildInputsForSameOrg(
                        org,
                        orgUsers,
                        systemRoles
                    )
                    await expect(getAddOrganizationRolesToUsers(input).run()).to
                        .be.fulfilled
            ...
            context('.generateEntityMaps', () => {
                ...
                beforeEach(async () => {
                    const data = await generateDataForEntityMaps()
                    orgMemberships = data.orgMemberships
                    otherOrgMemberships = data.otherOrgMemberships
                ...
                it('returns membership roles', async () => {
                    const {
                        input,
                        expectedMap,
                    } = await prepareDataForCheckMembershipRoles(
                        orgMemberships,
                        otherOrgMemberships,
                        otherOrg,
                        otherRoles,
                    ...
                    checkMembershipRoles(entityMaps, expectedMap)
                })
        ...
        context('RemoveOrganizationRolesFromUsers', () => {
            async function checkRolesRemoved(
                input: RemoveOrganizationRolesFromUserInput[]
            ) {
                const membershipRoles = await getMembershipRoles(input)
                input.forEach((i, index) => {
                    const roles = membershipRoles[index]
                    expect(roles).to.exist
                    expect(roles).to.not.have.lengthOf(i.roleIds.length)
                    expect(
                        roles.map((r) => r.role_id)
                    ).to.not.have.same.members(i.roleIds)
                })
            }

            beforeEach(async () => {
                const orgMemberships = await OrganizationMembership.find()
                orgMemberships.forEach(
                    (m) => (m.roles = Promise.resolve(orgRoles))
                )
            ...
            context('.run', () => {
                it('can remove roles from organization memberships', async () => {
                    const input = buildDefaultInputs()
                    await expect(
                        getRemoveOrganizationRolesFromUsers(input).run()
                    ).to.be.fulfilled

                    await checkRolesRemoved(input)
                })

                it('makes the expected number of db calls', async () => {
                    const input = buildDefaultInputs()
                    connection.logger.reset()
                    await getRemoveOrganizationRolesFromUsers(input).run()

                    expect(connection.logger.count).to.equal(
                        8,
                        'preload: 4, authorize: 1, save: 3'
                    )
                })
            })
            ...
            context('.generateEntityMaps', () => {
                ...
                it('returns membership roles', async () => {
                    ...
                    const mutation = getRemoveOrganizationRolesFromUsers(input)
                    const entityMaps = await mutation.generateEntityMaps(input)
                    checkMembershipRoles(entityMaps, expectedMap)
                })
            })
            ...
            context('.validate', async () => {
                async function validate(
                    ...
                    const mutation = getRemoveOrganizationRolesFromUsers(
                        input,
                        admin
                    )
                ...
                context(
                    'when the given role does not exist for the given user in the given organization',
                    () => {
                        let nonExistentRoleInMembership: Role
                        ...
                        beforeEach(async () => {
                            ...
                            nonExistentRoleInMembership = membershipRoles.pop()!
                            membership.roles = Promise.resolve(membershipRoles)
                            await membership.save()
                        })

                        it('produces error for a non existing role in the given organization membership', async () => {
                            const input = [
                                buildSingleInput(org, orgUsers[0], [
                                    nonExistentRoleInMembership,
                                ]),
                            ]
                            ...
                            const expectedErrors = [
                                createEntityAPIError(
                                    ...
                                    0,
                                    'Role',
                                    nonExistentRoleInMembership.role_id,
                                    'Organization',
                                    org.organization_id
                                ),
                            ]
