...
interface IChildConnectionRequest<Node> {
    ...
    // a generated ID to identify this request
    stringId: string
...
export const childConnectionLoader = async <Entity extends BaseEntity, Node>(
    ...
    const requests: IChildConnectionRequest<Node>[] = []
    for (const key of keys) {
        const id = getRequestIdentifier(key)
        const request = requests.find((r) => getRequestIdentifier(r.key) === id)
        if (!request) {
            requests.push({
                stringId: id,
                key,
    ...
    // resolve all child connections independently per request
