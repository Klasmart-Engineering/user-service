...
const port = getEnv({ name: 'PORT', orDefault: '8080' })
...
Sentry.init({
    ...
    environment: getEnv({ name: 'NODE_ENV', orDefault: 'not-specified' }),
    release: 'kidsloop-users-gql@' + getEnv({name: 'npm_package_version'}),
})
